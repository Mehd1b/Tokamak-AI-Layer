...
/**
 * @title TALIdentityRegistryV3
 * @notice Content-hash commitment pattern for tamper-evident agent URIs
 * @dev UUPS upgrade of TALIdentityRegistryV2. Storage layout preserves V1+V2 exactly,
 *      with new variables consuming 3 slots from the V2 `__gap[28]` → `__gap[25]`.
 *
 * V3 Additions:
 * - Content hash commitment: keccak256 of canonical registration JSON stored on-chain
 * - Critical fields hash: keccak256 of security-critical subset (services, capabilities, etc.)
 * - Content version counter: tracks number of content updates
 * - Agents that opt into content hashing are permanently locked into the hash-aware update path
 * - Legacy agents (V1/V2) continue to work unchanged via updateAgentURI()
 */
contract TALIdentityRegistryV3 is
    ERC721Upgradeable,
    ...
    ITALIdentityRegistry
{
    ...
    // =====================================================================
    // V2 STORAGE — Appended in place of V1 __gap slots (40 → 28)
    // =====================================================================
    ...
    // =====================================================================
    // V3 STORAGE — Appended in place of V2 __gap slots (28 → 25)
    // =====================================================================
    ...
    // ============ Storage Gap (reduced from 28 to 25) ============
    ...
    /// @notice Emitted when V3 is initialized
    event V3Initialized();
    ...
    /**
     * @notice V2 initializer — called once after proxy upgrade from V1 to V2
     */
    ...
    /**
     * @notice V3 initializer — called once after proxy upgrade from V2 to V3
     * @dev No configuration parameters needed; V3 storage mappings default to zero/empty.
     */
    ...
    function updateAgentURI(uint256 agentId, string calldata newURI) external {
        ...
        // V3: Block legacy updates for agents with content hash commitment
    ...
    // =====================================================================
    // V3 FUNCTIONS — Content Hash Commitment
    // =====================================================================
    ...
    /**
     * @notice Check if an agent has a content hash commitment
     * @param agentId The agent ID
     * @return True if the agent has opted into content hash verification
     */
    function hasContentCommitment(uint256 agentId) external view returns (bool) {
        return _contentVersion[agentId] > 0;
    }
