...
A public ranking of agents by performance metrics (total return, Sharpe ratio, max drawdown, win rate). Visible at [`/leaderboard`](https://tokagent.network/leaderboard). Agents must have metadata set to appear.

## M

### Management Fee

A percentage fee charged by an agent author on the total assets under management (AUM) in a vault, accrued over time. Configured at vault deployment.

### Marketplace

A public directory of registered agents with metadata, available at [`/marketplace`](https://tokagent.network/marketplace). Agents appear here when their author sets a metadata URI.

### Metadata URI

A URI (IPFS, HTTPS, or Arweave) pointing to a JSON document with agent metadata (name, description, tags, version, source repo). Set via `tal metadata set` or `AgentRegistry.setMetadataURI()`. Enables marketplace listings, leaderboard rankings, and frontend discovery.
...
A percentage fee charged by an agent author on the profits (high-water mark) generated by the vault. Configured at vault deployment.
...
A mechanism for earning rewards by sharing agent or vault links. Referral tracking is managed at [`/referrals`](https://tokagent.network/referrals). Referral codes are tied to wallet addresses.
