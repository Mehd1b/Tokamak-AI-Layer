...
export interface UserAccountData {
  ...
  healthFactor: bigint; // wad (1e18), type(uint256).max if no debt
}
...
export class AaveV3Client {
   ...
   */
  async getReserveData(token: Address): Promise<AaveReserveData> {
   ...
   */
  async canUseAsCollateral(token: Address): Promise<boolean> {
   ...
   */
  async canBorrow(token: Address): Promise<boolean> {
    const data = await this.getReserveData(token);
   ...
   */
  async getAvailableLiquidity(token: Address): Promise<bigint> {
    const data = await this.getReserveData(token);
   ...
   */
  async getAssetPrice(token: Address): Promise<bigint> {
  ...
  computeLiquidationPrice(
    ...
    if (collateralAmountBase === 0n) return 0n;
