...
export class PositionManager {
  private readonly positions: Map<string, LeveragedPosition> = new Map();
  private readonly aaveClient: AaveV3Client;
  private readonly lendingBuilder: LendingBuilder;
  ...
  buildCloseTransactions(
    ...
    logger.info(
      { positionId, direction: position.direction },
      "Building close transactions for position",
    );
   ...
   */
  async checkHealthFactor(user: Address): Promise<number> {
