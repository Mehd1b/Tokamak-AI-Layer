...
import type { PoolData, QuantScore, TradeRequest, DirectionalScore } from "@tal-trading-agent/shared";
...
export class TokenScorer {
   ...
   */
  private computeShortScore(score: QuantScore): number {
    const { indicators, defiMetrics, dataQuality } = score;
   ...
   */
  private rsiToShortSignal(rsi: number): number {
    if (rsi >= 80) return 90;  // Strongly overbought - strong short
    if (rsi >= 70) return 75;  // Overbought - short signal
    if (rsi >= 55) return 60;  // Slightly above neutral
    if (rsi >= 45) return 50;  // Neutral
    if (rsi >= 30) return 40;  // Slightly below neutral
    if (rsi >= 20) return 25;  // Oversold - weak short
   ...
   */
  private macdToShortSignal(macd: {
    value: number;
    signal: number;
    histogram: number;
  }): number {
   ...
   */
  private momentumToShortSignal(momentum: number): number {
    const clamped = Math.min(20, Math.max(-20, momentum));
   ...
   */
  private adxToShortSignal(adx: { adx: number; plusDI: number; minusDI: number }): number {
    if (adx.adx < 20) return 50; // No trend - neutral
   ...
   */
  private aroonToShortSignal(aroon: { up: number; down: number; oscillator: number }): number {
   ...
   */
  private stochRsiToShortSignal(stochRsi: { k: number; d: number; raw: number }): number {
    if (stochRsi.k >= 80) return 85;  // Overbought - strong short
    if (stochRsi.k >= 70) return 70;  // Mildly overbought
    if (stochRsi.k >= 30) return 50;  // Neutral
    if (stochRsi.k >= 20) return 30;  // Mildly oversold
   ...
   */
  private williamsRToShortSignal(wr: number): number {
    if (wr >= -20) return 85;  // Overbought - strong short
    if (wr <= -80) return 15;  // Oversold - weak short
   ...
   */
  private rocToShortSignal(roc: number): number {
    const clamped = Math.min(30, Math.max(-30, roc));
    // Invert: -30 -> 100, +30 -> 0
    return ((-clamped + 30) / 60) * 100;
  }
   ...
   */
  private atrToShortSignal(atr: { atr: number; atrPercent: number }): number {
   ...
   */
  private hvToShortSignal(hv: { dailyVol: number; annualizedVol: number }): number {
   ...
   */
  private vwapDeviationToShortSignal(dev: number): number {
    const clamped = Math.min(10, Math.max(-10, dev));
   ...
   */
  private bollingerPositionToShortSignal(bp: { percentB: number; bandwidth: number }): number {
    let score: number;
   ...
   */
  private trendStrengthToShortSignal(trendStrength: number): number {
