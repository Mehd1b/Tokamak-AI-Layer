...
import type { TokenMeta, TokenCategory } from "@tal-trading-agent/shared";
...
const RISK_CATEGORIES: Record<string, TokenCategory[]> = {
...
const PROMPT_CATEGORY_MAP: Record<string, TokenCategory[]> = {
  ai: ["ai-data"],
  artificial: ["ai-data"],
...
export class TokenPreFilter {
  ...
  async preFilter(
    ...
    logger.info(
      { totalTokens: allTokens.length, riskTolerance: options.riskTolerance, maxCandidates },
      "Starting token pre-filter",
    );

    // Step 1: Exclude quote token and stablecoins if budget is stablecoin
    let filtered = allTokens.filter(
      (t) => t.address.toLowerCase() !== quoteToken.toLowerCase(),
    );

    const quoteIsStablecoin = allTokens.find(
      (t) => t.address.toLowerCase() === quoteToken.toLowerCase() && t.category === "stablecoin",
    );
    ...
    const scored = filtered.map((t) => {
      let score = 0;
      if (t.isDefault) score += 10;
      if (boostedCategories.has(t.category)) score += 20;
    ...
    logger.info(
      { filtered: result.length, fromTotal: allTokens.length },
      "Token pre-filter complete",
    );

    return result;
  }

  private async batchPriceCheck(tokens: TokenMeta[]): Promise<TokenMeta[]> {
    try {
  ...
  private async batchPoolCheck(tokens: TokenMeta[], quoteToken: Address): Promise<TokenMeta[]> {
    ...
    try {
      const calls = tokens.map((t) => ({
        ...
        args: [t.address, quoteToken, 3000] as const,
      }));
