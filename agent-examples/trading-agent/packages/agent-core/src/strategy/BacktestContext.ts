...
// ── Minimal backtest result types (avoid cross-package dependency) ──
...
let cachedContext: BacktestContext | null = null;
let cacheKey: string | null = null;

function classifyRegime(totalReturnPct: number): MarketRegime {
  if (totalReturnPct > 10) return "bull";
  if (totalReturnPct < -10) return "bear";
  return "sideways";
}

function formatPromptSection(m: BacktestMetrics, regime: MarketRegime): string {
  const regimeLabel = regime === "bull" ? "bullish" : regime === "bear" ? "bearish" : "sideways";
  const oppositeAdvice =
    regime === "bear"
 ...
 */
export async function loadBacktestContext(): Promise<BacktestContext | null> {
  ...
  for (const filePath of searchPaths) {
    ...
    try {
      ...
      const data = JSON.parse(raw) as BacktestMetrics;
      ...
      const ctx: BacktestContext = { metrics: data, regime, promptSection };
      cachedContext = ctx;
      cacheKey = filePath;

      logger.info(
        { filePath, regime, totalReturn: data.totalReturnPct, sharpe: data.sharpeRatio },
        "Loaded backtest context",
      );
 ...
 */
export function getCachedBacktestContext(): BacktestContext | null {
  return cachedContext;
}
