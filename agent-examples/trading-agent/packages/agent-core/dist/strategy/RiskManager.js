...
import { getCachedBacktestContext } from "./BacktestContext.js";
    ...
    validateStrategy(strategy) {
        ...
        for (const trade of strategy.trades) {
            if (trade.leverageConfig) {
                ...
                if (trade.leverageConfig.leverageMultiplier > maxLev) {
                    errors.push(`Trade ${trade.tokenIn} -> ${trade.tokenOut}: leverage ${trade.leverageConfig.leverageMultiplier}x exceeds max (${maxLev}x)`);
                }
            }
            // Check if shorts are allowed
            if (trade.direction === "short" && params.allowShorts === false) {
                errors.push(`Short position on ${trade.tokenOut} is not allowed for ${strategy.request.riskTolerance} risk tolerance`);
            }
        }
        ...
        if (strategy.riskMetrics.healthFactor !== undefined) {
            ...
            if (strategy.riskMetrics.healthFactor < minHf) {
                errors.push(`Health factor ${strategy.riskMetrics.healthFactor.toFixed(2)} below minimum (${minHf})`);
            }
        }
        ...
        if (leveragedTrades.length > 0) {
            const totalLeveragedAmount = leveragedTrades.reduce((sum, t) => sum + t.amountIn * BigInt(Math.round((t.leverageConfig?.leverageMultiplier ?? 1) * 100)) / 100n, 0n);
            if (totalLeveragedAmount > budget * 2n) {
                warnings.push(`Total leveraged exposure (${totalLeveragedAmount.toString()}) exceeds 2x budget`);
            }
        }
        ...
        if (btCtx) {
            ...
            if (bt.maxDrawdownPct > 15) {
                warnings.push(`Backtest showed ${bt.maxDrawdownPct.toFixed(1)}% max drawdown — consider tighter position sizing`);
            }
            if (bt.sharpeRatio < 0) {
                warnings.push(`Backtest Sharpe ratio is negative (${bt.sharpeRatio.toFixed(2)}) — signal pipeline underperformed risk-free rate`);
            }
            if (bt.profitFactor < 1.0) {
                warnings.push(`Backtest profit factor < 1.0 (${bt.profitFactor.toFixed(2)}) — gross losses exceeded gross profits`);
            }
        }
    ...
    adjustForRisk(strategy) {
        ...
        for (let i = 0; i < finalTrades.length; i++) {
            ...
            if (trade.leverageConfig) {
                const maxLev = params.maxLeverage ?? 1;
                if (trade.leverageConfig.leverageMultiplier > maxLev) {
                    this.log.info({ tokenOut: trade.tokenOut, original: trade.leverageConfig.leverageMultiplier, capped: maxLev }, "Capping leverage multiplier");
                    finalTrades[i] = {
                        ...trade,
                        leverageConfig: {
        ...
        if (btCtx) {
            ...
            if (bt.sharpeRatio < 0) {
                this.log.info({ sharpe: bt.sharpeRatio }, "Negative backtest Sharpe — scaling down positions by 25%");
                for (let i = 0; i < finalTrades.length; i++) {
                    const trade = finalTrades[i];
                    finalTrades[i] = {
                        ...trade,
            ...
            if (bt.profitFactor < 1.0) {
                this.log.info({ profitFactor: bt.profitFactor }, "Backtest profit factor < 1.0 — additional 15% position reduction");
                for (let i = 0; i < finalTrades.length; i++) {
                    const trade = finalTrades[i];
                    finalTrades[i] = {
                        ...trade,
