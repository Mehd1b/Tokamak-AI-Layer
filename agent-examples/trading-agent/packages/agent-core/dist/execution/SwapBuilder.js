...
export class SwapBuilder {
    ...
    buildExactInputSingle(params) {
        const { tokenIn, tokenOut, fee, recipient, amountIn, slippageBps, } = params;
    ...
    buildExactInput(params) {
        ...
        if (fees.length !== path.length - 1) {
            throw new Error(`Fee array length (${fees.length}) must equal path length - 1 (${path.length - 1})`);
        }
     ...
     */
    buildFromTradeAction(action, recipient) {
        ...
        if (action.route.length > 2) {
            ...
            const fees = Array.from({ length: action.route.length - 1 }, () => action.poolFee);
     ...
     */
    inferSlippageBps(amountIn, minAmountOut) {
        if (amountIn === 0n)
            return 100; // default 1%
        // slippageBps = (1 - minAmountOut/amountIn) * 10000
        const slippage = Number(((amountIn - minAmountOut) * BPS_DENOMINATOR) / amountIn);
...
//# sourceMappingURL=SwapBuilder.js.map