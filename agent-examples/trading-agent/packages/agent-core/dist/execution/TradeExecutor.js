...
export class TradeExecutor {
    ...
    async estimateGas(swap) {
        try {
            ...
            logger.info({ to: swap.to, estimate: estimate.toString() }, "Gas estimated");
            return estimate;
        }
        catch (error) {
            logger.warn({ error, to: swap.to }, "Gas estimation failed, using swap default");
     ...
     */
    async broadcastSignedTx(serializedTx) {
     ...
     */
    async waitForReceipt(txHash, strategyId = "unknown") {
    ...
    parseSwapAmounts(receipt) {
        ...
        for (const log of receipt.logs) {
            ...
            try {
                ...
                const data = log.data;
                if (data.length < 130)
                    continue; // 0x + 64 hex chars * 2 = 130 min
