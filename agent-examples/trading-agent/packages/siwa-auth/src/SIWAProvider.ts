...
export class SIWAProvider {
  ...
  constructor(config: { domain: string; sessionTtl: number }) {
  ...
  validateSession(session: SIWASession): boolean {
    ...
    if (session.expiresAt <= now) {
      logger.debug(
        { address: session.address, expiresAt: session.expiresAt, now },
        "Session expired",
      );
      return false;
    }
    ...
    if (session.issuedAt > now) {
      logger.warn(
        { address: session.address, issuedAt: session.issuedAt },
