...
// ---------------------------------------------------------------------------
// ERC-8004 Registration File (built inline, not via RegistrationBuilder)
// ---------------------------------------------------------------------------
...
export class TradingAgentTAL {
  ...
  async registerTradingAgent(
    ...
    const registrationFile: ERC8004RegistrationFile = {
      ...
      description:
        "Autonomous quantitative trading agent on the Tokamak AI Layer. Accepts natural-language prompts and infers trading parameters automatically. Analyzes Uniswap V3 pool liquidity and DeFiLlama market data across 9 technical and DeFi indicators, then generates optimized strategies via Claude with extended thinking. Supports four trading modes (scalp, swing, position, investment) with DCA scheduling, portfolio rebalancing, and configurable exit criteria. Produces unsigned swap calldata for trustless execution and downloadable self-executing trading bots. On-chain fee escrow via TaskFeeEscrow.",
      active: true,
      services: {
      ...
      tal: {
        capabilities: [
          ...
          },
          {
            ...
            description:
              "Broadcasts user-signed transactions to Ethereum mainnet. Parses Uniswap V3 Swap event logs for actual traded amounts. Requires SIWA authentication.",
            inputSchema: {
              type: "object",
              properties: {
          ...
          },
          {
            ...
            description:
              "Generates a self-contained Node.js bot (.zip) with auto-executing stop-loss/take-profit/trailing-stop listener, ERC-20 approval handling, DCA scheduler, portfolio rebalancer, and pre-configured strategy parameters.",
            inputSchema: {
              type: "object",
              properties: {
                strategyId: { type: "string" },
    ...
    this.log.info(
      { agentURI },
      "Registration file prepared, calling register()",
    );
    ...
    this.log.info(
      { agentId: agentId.toString(), txHash },
      "Agent registered on TAL",
    );
  ...
  async getAgentInfo(
    agentId: bigint,
  ): Promise<{ owner: Address; agentURI: string; operator: Address }> {
  ...
  async submitTradeResult(
    ...
    const scaledValue = BigInt(
      Math.round(feedback.value * 10 ** VALUE_DECIMALS),
    );
  ...
  async submitTradeFailure(
    ...
    this.log.warn(
      { agentId: agentId.toString(), reason: params.reason },
      "Submitting trade failure feedback (negative reputation)",
    );
  ...
  async getReputation(
    agentId: bigint,
  ): Promise<{ feedbackCount: bigint; averageScore: number }> {
  ...
  async requestValidation(params: {
    ...
    outputHash: Hex;
    bounty: bigint;
  }): Promise<{ requestHash: Hex; txHash: Hash }> {
    this.requireWallet();
    ...
    if (validationEvents.length === 0) {
      throw new Error(
        "ValidationRequested event not found in transaction receipt",
      );
    }

    const requestHash = validationEvents[0]!.args.requestHash as Hex;
  ...
  async getValidationStatus(
    ...
    const [request, response] = result as unknown as [
      { status: number },
      { validator: Address; score: number; timestamp: bigint },
    ];
  ...
  async setAgentFee(
    ...
    this.log.info(
      { agentId: agentId.toString(), feePerTask: feePerTask.toString() },
      "Setting agent fee on TaskFeeEscrow",
    );
   ...
   */
  async confirmTask(taskRef: Hex): Promise<Hash> {
    this.requireWallet();
  ...
  async claimFees(agentId: bigint): Promise<Hash> {
    ...
    this.log.info(
      { agentId: agentId.toString() },
      "Claiming accumulated fees from escrow",
    );
   ...
   */
  async getEscrowStatus(taskRef: Hex): Promise<TaskEscrowData> {
    const result = (await this.publicClient.readContract({
      address: TAL_CONTRACTS.taskFeeEscrow,
      abi: taskFeeEscrowAbi,
    ...
    return {
      ...
      status: result.status as EscrowTaskStatus,
    };
  }
   ...
   */
  async isTaskPaid(taskRef: Hex): Promise<boolean> {
  ...
  private requireWallet(): void {
    if (!this.walletClient) {
      throw new Error(
        "WalletClient required for write operations. Provide walletClient in constructor params.",
      );
    }
  ...
  private async uploadRegistration(
    ...
    this.log.warn(
      "No Pinata credentials configured, using inline data URI as placeholder",
    );
    const encoded = Buffer.from(JSON.stringify(file)).toString("base64");
    return `data:application/json;base64,${encoded}`;
  }

  private parseAgentIdFromReceipt(receipt: {
    logs: readonly {
