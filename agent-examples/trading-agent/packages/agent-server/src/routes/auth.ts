...
export async function authRoutes(app: FastifyInstance, ctx: AppContext) {
  ...
  app.post<{ Body: typeof VerifyBody.static }>(
    "/api/v1/auth/verify",
    ...
    async (req, reply) => {
      ...
      const valid = await ctx.siwaProvider.verifySignature(
        message,
        signature as Hex,
        address as Address,
      );
      ...
      // Create session
      const session = ctx.siwaProvider.createSession(
        address as Address,
        BigInt(agentId ?? "0"),
        chainId ?? 1,
        signature as Hex,
  ...
  app.post("/api/v1/auth/refresh", async (req, reply) => {
    ...
    try {
      ...
      const newSession = ctx.siwaProvider.createSession(
        oldSession.address,
        oldSession.agentId,
        oldSession.chainId,
        oldSession.signature,
      );

      const token = Buffer.from(
        JSON.stringify(newSession, (_key, value) =>
          typeof value === "bigint" ? value.toString() : value,
        ),
      ).toString("base64");
