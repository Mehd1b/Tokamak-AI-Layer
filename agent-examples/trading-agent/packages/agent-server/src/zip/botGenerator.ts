    ...
    if (isInvestment && strategy.investmentPlan) {
      ...
      if (plan.rebalancing) {
        ...
        if (plan.rebalancing.driftThresholdPercent != null) {
          envLines.push(
            `DRIFT_THRESHOLD_PERCENT=${plan.rebalancing.driftThresholdPercent}`,
          );
        }
      }
      ...
      if (plan.exitCriteria) {
        ...
        if (plan.exitCriteria.takeProfitPercent != null) {
          envLines.push(
            `EXIT_TAKE_PROFIT_PERCENT=${plan.exitCriteria.takeProfitPercent}`,
          );
        }
        if (plan.exitCriteria.stopLossPercent != null) {
          envLines.push(
            `EXIT_STOP_LOSS_PERCENT=${plan.exitCriteria.stopLossPercent}`,
          );
        }
        if (plan.exitCriteria.timeExitMonths != null) {
          envLines.push(
            `EXIT_TIME_MONTHS=${plan.exitCriteria.timeExitMonths}`,
          );
        }
      }
    }
