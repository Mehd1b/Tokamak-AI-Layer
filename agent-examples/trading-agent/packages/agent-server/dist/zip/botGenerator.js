 ...
 */
export async function generateBotZip(strategy) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const isDCA = isInvestment && strategy.investmentPlan?.entryStrategy === "dca";
        const hasRebalancing = isInvestment && !!strategy.investmentPlan?.rebalancing;
        // ── package.json ──────────────────────────────────────
        ...
        }, null, 2), { name: `${prefix}/package.json` });
        // ── tsconfig.json ─────────────────────────────────────
        archive.append(JSON.stringify({
            compilerOptions: {
                target: "ES2022",
            ...
            include: ["src"],
        }, null, 2), { name: `${prefix}/tsconfig.json` });
        // ── .env.example ──────────────────────────────────────
        const tradesJson = JSON.stringify(strategy.trades.map((t) => ({
            action: t.action,
            tokenIn: t.tokenIn,
            tokenOut: t.tokenOut,
        ...
        indexLines.push(``, `async function main() {`, `  console.log("Trading Bot - Strategy ${strategy.id}");`, `  console.log("Mode: ${strategy.mode}");`, `  console.log("Market condition: ${strategy.analysis.marketCondition}");`, `  console.log("Confidence: ${strategy.analysis.confidence}%");`, `  console.log("Expected return: ${strategy.estimatedReturn.expected}%");`, `  console.log("");`, ``, `  const interval = parseInt(process.env.CHECK_INTERVAL ?? "60", 10);`, `  const mode = process.env.STRATEGY_MODE ?? "${strategy.mode}";`, `  const entryStrategy = process.env.ENTRY_STRATEGY ?? "${strategy.investmentPlan?.entryStrategy ?? "lump-sum"}";`);
        if (isDCA) {
            indexLines.push(``, `  if (mode === "investment" && entryStrategy === "dca") {`, `    console.log("Investment mode: DCA entry\\n");`, `    // DCA buys over time instead of immediate execution`, `    startDCA(interval);`, `    // Start listener for exit criteria`, `    startListener(interval);`);
            if (hasRebalancing) {
                indexLines.push(`    // Start rebalancer`, `    startRebalancer(interval);`);
            }
        ...
        indexLines.push(`    // Execute initial trades immediately`, `    await execute();`, `    // Start listener for stop-loss / take-profit / trailing-stop`, `    startListener(interval);`);
        if (hasRebalancing && !isDCA) {
            indexLines.push(`    // Start rebalancer if in investment mode`, `    if (mode === "investment") {`, `      startRebalancer(interval);`, `    }`);
        }
        indexLines.push(`  }`, `}`, ``, `main().catch((err) => {`, `  console.error("Fatal error:", err);`, `  process.exit(1);`, `});`);
        ...
        if (isInvestment) {
            readmeLines.push(`## Investment Mode`, ``, `This bot is configured for long-term investment with:`, ``);
            if (isDCA) {
                readmeLines.push(`### DCA (Dollar Cost Averaging)`, ``, `Instead of buying everything at once, the bot spreads purchases over time:`, `- **Frequency**: \`DCA_FREQUENCY\` (${strategy.investmentPlan?.dcaSchedule?.frequency ?? "weekly"})`, `- **Total periods**: \`DCA_TOTAL_PERIODS\` (${strategy.investmentPlan?.dcaSchedule?.totalPeriods ?? 52})`, `- **Amount per period**: \`DCA_AMOUNT_PER_PERIOD_PERCENT\`% of original budget`, ``, `DCA state is persisted in \`state.json\` so the bot can be restarted safely.`, ``);
            }
            if (hasRebalancing) {
                const rb = strategy.investmentPlan.rebalancing;
                readmeLines.push(`### Rebalancing`, ``, `The bot automatically rebalances your portfolio to maintain target allocations:`, `- **Type**: ${rb.type}`);
                if (rb.type === "drift") {
                    readmeLines.push(`- **Drift threshold**: ${rb.driftThresholdPercent ?? 5}% — rebalances when any position drifts beyond this`);
                }
                if (rb.frequency) {
        ...
        readmeLines.push(`## Risk`, `- Max drawdown: ${strategy.riskMetrics.maxDrawdown}%`, `- Position size: ${strategy.riskMetrics.positionSizePercent}%`, `- Stop-loss configured: Yes`, ``, `## Warning`, ``, `This bot executes real transactions with real funds. Running it requires a funded`, `wallet with your private key in the \`.env\` file. Review all settings carefully`, `before running. Use at your own risk.`, ``, `---`, `Generated by [TAL Trading Agent](https://github.com/tokamak-network)`);
