export async function strategyRoutes(app, ctx) {
    ...
    app.get("/api/v1/strategy/:id/risk", async (req, reply) => {
    ...
    app.post("/api/v1/strategy/:id/approve", async (req, reply) => {
        const strategy = ctx.strategyCache.get(req.params.id);
        if (!strategy) {
            return reply.code(404).send({ error: "Strategy not found" });
        }
        // Build unsigned swaps for the client to sign
        const unsignedSwaps = strategy.trades.map((trade) => ctx.swapBuilder.buildFromTradeAction(trade, strategy.request.walletAddress));
        return reply.send({
            strategyId: req.params.id,
            approved: true,
