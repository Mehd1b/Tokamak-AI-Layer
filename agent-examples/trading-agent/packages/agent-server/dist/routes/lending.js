...
export async function lendingRoutes(app, ctx) {
    // ── GET /api/v1/lending/reserves/:token ───────────────
    app.get("/api/v1/lending/reserves/:token", async (req, reply) => {
        const { token } = req.params;
        if (!isAddress(token)) {
            return reply.code(400).send({ error: "Invalid token address" });
        }
        try {
            const data = await ctx.aaveV3Client.getReserveData(token);
            return reply.send({
                token,
                ltv: data.ltv,
    ...
    app.get("/api/v1/lending/account/:address", async (req, reply) => {
        const { address } = req.params;
        if (!isAddress(address)) {
            return reply.code(400).send({ error: "Invalid wallet address" });
        }
        try {
            const data = await ctx.aaveV3Client.getUserAccountData(address);
    ...
    app.post("/api/v1/lending/simulate", async (req, reply) => {
        ...
        try {
            const [collateralReserve, debtReserve, collateralPrice, debtPrice] = await Promise.all([
                ctx.aaveV3Client.getReserveData(collateralToken),
                ctx.aaveV3Client.getReserveData(debtToken),
                ctx.aaveV3Client.getAssetPrice(collateralToken),
                ctx.aaveV3Client.getAssetPrice(debtToken),
            ]);
            ...
            const healthFactor = ctx.aaveV3Client.computeHealthFactor(collateralValueUsd, debtValueUsd, collateralReserve.liquidationThreshold);
            const liquidationPrice = ctx.aaveV3Client.computeLiquidationPrice(collateralValueUsd, debtValueUsd, collateralReserve.liquidationThreshold);
