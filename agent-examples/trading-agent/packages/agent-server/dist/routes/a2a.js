...
export async function a2aRoutes(app, ctx) {
    ...
    app.get("/api/agents/trader", async (req, reply) => {
        ...
        return reply.send({
            ...
            description: "AI-powered quantitative trading agent on the Tokamak Agent Layer. Analyzes Uniswap V3 pools, generates LLM-driven strategies with risk management, and produces unsigned swap calldata for execution.",
            ...
            skills: [
                {
                    id: "trade-analysis",
                    name: "Trade Analysis",
                    description: "Accepts a natural-language trading prompt with budget, horizon, and risk tolerance. Scores tokens via on-chain Uniswap V3 pool data and DeFiLlama quant indicators, then generates an optimized trading strategy using Claude. Returns the strategy with unsigned swap calldata ready for wallet signing.",
                ...
                },
                {
                    id: "task-status",
                    name: "Task Status",
                    description: "Check the status and results of a previously submitted trade analysis task.",
                    tags: ["status", "query"],
