...
export async function tradeRoutes(app, ctx) {
    const siwaAuth = siwaAuthMiddleware(ctx.siwaProvider);
    // ── POST /api/v1/trade/analyze ─────────────────────────
    app.post("/api/v1/trade/analyze", { schema: { body: AnalyzeBody } }, async (req, reply) => {
        const body = req.body;
        ...
        const spotTrades = strategy.trades.filter((t) => !t.positionType || t.positionType === "spot_long");
        const unsignedSwaps = spotTrades.map((trade) => ctx.swapBuilder.buildFromTradeAction(trade, request.walletAddress));
        // 6. Build lending transactions for leveraged/short trades
        const leveragedTrades = strategy.trades.filter((t) => t.positionType && t.positionType !== "spot_long");
    ...
    app.post("/api/v1/trade/execute", { preHandler: [siwaAuth], schema: { body: ExecuteBody } }, async (req, reply) => {
        ...
        ctx.logger.info({ strategyId, wallet: session.address }, "Trade execution requested");
        // Broadcast the pre-signed transaction
        const { txHash } = await ctx.tradeExecutor.broadcastSignedTx(signedTransaction);
    ...
    app.get("/api/v1/trade/:strategyId", async (req, reply) => {
    ...
    app.get("/api/v1/trade/:strategyId/status", async (req, reply) => {
    ...
    app.get("/api/v1/trade/:strategyId/download", async (req, reply) => {
        const strategy = ctx.strategyCache.get(req.params.strategyId);
        if (!strategy) {
            return reply.code(404).send({ error: "Strategy not found" });
        }
        ...
        reply.header("Content-Disposition", `attachment; filename="trading-bot-${strategy.id}.zip"`);
