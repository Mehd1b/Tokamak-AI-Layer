...
export class ReportGenerator {
    ...
    printReport(result) {
        ...
        if (result.trades.length > 0) {
            ...
            if (topWins.length === 0) {
                ...
                for (const t of topWins) {
                    console.log(`  ${t.symbol} ${t.direction} | P&L: $${t.pnl.toFixed(2)} (${sign(t.pnlPercent)}%) | ${t.holdingBars} bars | ${t.exitReason}`);
                }
            }
            console.log("\n--- Top 5 Losing Trades ---");
        ...
        if (result.equityCurve.length > 2) {
            ...
            console.log(`  Start: $${config.initialCapital.toLocaleString()} -> End: $${final.equity.toFixed(2)}`);
        }
        console.log("\n" + "=".repeat(70) + "\n");
    }
    /**
    ...
    sparkline(values) {
        ...
        const range = max - min;
        if (range === 0)
            return chars[4].repeat(sampled.length);
        return sampled
            .map((v) => {
