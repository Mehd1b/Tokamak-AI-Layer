...
export class BacktestEngine {
    ...
    async run(config) {
        ...
        logger.info({ bars: alignedTimestamps.length, tokens: priceMatrix.size }, "Data aligned, starting simulation");
        ...
        for (let barIdx = 0; barIdx < alignedTimestamps.length; barIdx++) {
            ...
            portfolio.checkOrders(currentPrices, barIdx, timestamp, barTimestamps, config.risk);
            ...
            for (const [token, prices] of priceMatrix) {
                ...
                const priceWindow = prices.slice(windowStart, windowEnd).filter((p) => p !== undefined && p > 0);
            ...
            for (const candidate of ranked) {
                if (portfolio.getOpenPositionCount() >= config.strategy.maxPositions)
                    break;
                if (portfolio.hasPositionFor(candidate.token))
                    continue;
                ...
                if (allowLongs && candidate.longScore >= config.strategy.entryThreshold) {
                    ...
                    portfolio.openPosition(candidate.token, symbol, "long", fillPrice, config.risk.maxPositionPct, currentEquity, candidate.atr, config.risk, barIdx);
                }
                // Short entry (if enabled, gated by trend filter)
                if (config.strategy.useShorts &&
                    allowShorts &&
                    !portfolio.hasPositionFor(candidate.token) &&
                    ...
                    const currentEquity = portfolio.computeEquity(currentPrices);
                    const symbol = tokenSymbols.get(candidate.token) ?? candidate.token.slice(0, 8);
                    portfolio.openPosition(candidate.token, symbol, "short", fillPrice, config.risk.maxPositionPct, currentEquity, candidate.atr, config.risk, barIdx);
                }
            }
            ...
            if (portfolio.isCircuitBreakerTriggered(config.risk.maxDrawdownPct)) {
                logger.warn({ bar: barIdx, drawdown: config.risk.maxDrawdownPct }, "Circuit breaker triggered — halting new trades");
                circuitBreakerHit = true;
            }
        }
        // Debug: trend filter stats
        if (config.strategy.trendFilter.enabled) {
            logger.info({ tfActive, tfNull, tfBlockedLongs, tfBlockedShorts }, "Trend filter stats");
        }
        ...
        portfolio.closeAllPositions(lastPrices, alignedTimestamps.length - 1, lastTimestamp, barTimestamps);
     ...
     */
    async loadAllPrices(config) {
        const tokenPrices = new Map();
        for (const token of config.tokens) {
            const bars = await this.dataLoader.loadPrices(token, config.startDate, config.endDate, config.barInterval);
            if (bars.length > 0) {
     ...
     */
    computeSMA(prices, barIdx, period) {
        if (barIdx < period - 1)
            return null;
        let sum = 0;
        let count = 0;
