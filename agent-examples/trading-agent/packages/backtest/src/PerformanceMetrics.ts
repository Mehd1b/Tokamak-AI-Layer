...
export class PerformanceMetrics {
  ...
  compute(
    ...
    trades: ClosedTrade[],
    tokenPrices: Map<string, PriceBar[]>,
  ): BacktestResult {
    ...
    const { maxDrawdownPct, maxDrawdownDurationBars, drawdownCurve } =
      this.computeDrawdown(equityCurve);
    ...
    // Trade stats
    const { winRate, profitFactor, avgWinPct, avgLossPct, largestWinPct, largestLossPct, avgHoldingBars } =
      this.computeTradeStats(trades);
   ...
   */
  private computeCAGR(initial: number, final: number, startDate: Date, endDate: Date): number {
   ...
   */
  private computeDrawdown(equityCurve: EquityPoint[]): {
    ...
    const drawdownCurve: { timestamp: number; drawdownPct: number }[] = [];
   ...
   */
  private computeAnnualizedVolatility(equityCurve: EquityPoint[], barInterval: string): number {
   ...
   */
  private computeDownsideDeviation(equityCurve: EquityPoint[], barInterval: string): number {
    const returns = this.computeReturns(equityCurve);
   ...
   */
  private computeTradeStats(trades: ClosedTrade[]): {
    winRate: number;
    profitFactor: number;
    avgWinPct: number;
    avgLossPct: number;
   ...
   */
  private computeBuyAndHold(tokenPrices: Map<string, PriceBar[]>, initialCapital: number): number {
  ...
  private computeReturns(equityCurve: EquityPoint[]): number[] {
  ...
  private barsPerYear(barInterval: string): number {
    switch (barInterval) {
      case "1h":
        return 8760;
      case "4h":
