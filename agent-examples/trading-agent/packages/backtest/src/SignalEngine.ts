...
export class SignalEngine {
  private readonly quant: QuantAnalysis;
...
function computeWeightedScore(signals: Record<string, number>): number {
  let weighted = 0;
  const weights = TECHNICAL_WEIGHTS as Record<string, number>;
...
function momentumToSignal(momentum: number): number {
...
function macdToSignal(macd: { value: number; signal: number; histogram: number }): number {
  const normalized = Math.tanh(macd.histogram * 10) * 50 + 50;
  return Math.min(100, Math.max(0, normalized));
}

function adxToSignal(adx: { adx: number; plusDI: number; minusDI: number }): number {
  if (adx.adx < 20) return 50;
  const trendStrength = Math.min(1, (adx.adx - 20) / 30);
  if (adx.plusDI > adx.minusDI) return 50 + trendStrength * 40;
  return 50 - trendStrength * 40;
}

function aroonToSignal(aroon: { up: number; down: number; oscillator: number }): number {
  return (aroon.oscillator + 100) / 2;
}

function stochRsiToSignal(stochRsi: { k: number; d: number; raw: number }): number {
  if (stochRsi.k <= 20) return 85;
  if (stochRsi.k <= 30) return 70;
  if (stochRsi.k <= 70) return 50;
  if (stochRsi.k <= 80) return 30;
  return 15;
}

function williamsRToSignal(wr: number): number {
  if (wr <= -80) return 85;
  if (wr >= -20) return 15;
  return 85 - ((wr + 80) / 60) * 70;
}

function rocToSignal(roc: number): number {
  const clamped = Math.min(30, Math.max(-30, roc));
  return ((clamped + 30) / 60) * 100;
}

function atrToSignal(atr: { atr: number; atrPercent: number }): number {
...
function hvToSignal(hv: { dailyVol: number; annualizedVol: number }): number {
  const annual = hv.annualizedVol;
  if (annual <= 0.3) return 75;
  if (annual <= 0.6) return 60;
  if (annual <= 1.0) return 40;
  if (annual <= 1.5) return 25;
  return 10;
}

function vwapDeviationToSignal(dev: number): number {
  const clamped = Math.min(10, Math.max(-10, dev));
  return ((clamped + 10) / 20) * 100;
}

function bollingerPositionToSignal(bp: { percentB: number; bandwidth: number }): number {
  let score: number;
  if (bp.percentB > 1) {
    score = 20;
...
function momentumToShortSignal(momentum: number): number {
  const clamped = Math.min(20, Math.max(-20, momentum));
  return ((-clamped + 20) / 40) * 100;
}

function rsiToShortSignal(rsi: number): number {
  if (rsi >= 80) return 90;
  if (rsi >= 70) return 75;
  if (rsi >= 55) return 60;
  if (rsi >= 45) return 50;
...
function macdToShortSignal(macd: { value: number; signal: number; histogram: number }): number {
  const normalized = Math.tanh(-macd.histogram * 10) * 50 + 50;
  return Math.min(100, Math.max(0, normalized));
}

function adxToShortSignal(adx: { adx: number; plusDI: number; minusDI: number }): number {
  if (adx.adx < 20) return 50;
  const trendStrength = Math.min(1, (adx.adx - 20) / 30);
  if (adx.minusDI > adx.plusDI) return 50 + trendStrength * 40;
  return 50 - trendStrength * 40;
}

function aroonToShortSignal(aroon: { up: number; down: number; oscillator: number }): number {
  return (-aroon.oscillator + 100) / 2;
}

function stochRsiToShortSignal(stochRsi: { k: number; d: number; raw: number }): number {
  if (stochRsi.k >= 80) return 85;
  if (stochRsi.k >= 70) return 70;
  if (stochRsi.k >= 30) return 50;
  if (stochRsi.k >= 20) return 30;
  return 15;
}

function williamsRToShortSignal(wr: number): number {
  if (wr >= -20) return 85;
  if (wr <= -80) return 15;
  return 15 + ((wr + 80) / 60) * 70;
}

function rocToShortSignal(roc: number): number {
  const clamped = Math.min(30, Math.max(-30, roc));
  return ((-clamped + 30) / 60) * 100;
}

function vwapDeviationToShortSignal(dev: number): number {
  const clamped = Math.min(10, Math.max(-10, dev));
  return ((clamped + 10) / 20) * 100;
}

function bollingerPositionToShortSignal(bp: { percentB: number; bandwidth: number }): number {
  let score: number;
  if (bp.percentB > 1) {
    score = 80;
  } else if (bp.percentB < 0) {
...
function trendStrengthToShortSignal(trendStrength: number): number {
